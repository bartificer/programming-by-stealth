...
The jq language allows you to combine boolean values using the `and`, and `or` operators. These will cast the values they process to booleans, and in jq, **everything that's not `false` or `null` is considered `true`**, and that includes `0`, empty arrays, and empty dictionaries:
...
This returns a surprising number of results, each of which has no `laureats` key at all, but instead has an `overallMotivation` key with an explanation that there was no prize awarded, and a description of what was done with the money instead. 
