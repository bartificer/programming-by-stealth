...
Before we switch our focus back to building statically generated websites with Jekyll, we need to take one more short diversion. Before we took a break from the topic, we described the mechanisms for developing the *look* of a Jekyll website, and after our break, we'll shift our focus to how Jekyll allows you to structure your content. **BART: previous sentence is confusing as it links to the next sentence**  In the modern world, that means ensuring the content you publish on your Jekyll sites produces effective link previews in social media apps. The [OpenGraph protocol](https://en.wikipedia.org/wiki/Open_Graph_protocol) was designed to solve this problem. It originated within what was then Facebook, but it has matured into a widely adopted standard.
...
Taking this approach lead to collecting almost all the code into a single include file used by all my layouts (`./docs/_includes/html_head_common.html`), and supporting front matter definitions includes, layouts, and pages.
...
Once we have our four arrays for the keywords coming from the site, page, layout, and include parameters, they get merged into a single array, joined into a coma-separated list, and if the list is not empty, rendered to the page:
...
The `podcast` keywords is coming from the site config file (`_config.yml`):
...
TO DO — insert OpenGraph code snippet
