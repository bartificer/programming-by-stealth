...
Before we switch our focus back to building statically generated websites with Jekyll, we need to take one more short diversion. Before we took a break from the topic, we described the mechanisms for developing the *look* of a Jekyll website, and after our break, we'll shift our focus to how Jekyll allows you to structure your content. **BART: confused - is the "how you structure your content" what you're saying in the next sentence? Then what's in between, as in today's lesson?** In the modern world, that means ensuring the content you publish on your Jekyll sites is properly indexed by search engines and looks good when embedded into social media posts. This means looking at two related but subtly different technologies — the traditional metadata headers developed early in the web's history to support search engines, and the much newer OpenGraph protocol developed to allow web pages to enrich their preview cards in social media posts.
...
There aren't many of these headers in the entire official specification, and not all of them are relevant to the kinds of websites we can build with content management systems like WordPress and Jekyll. Here's a list of the most relevant ones: **BART: I thought Jekyll isn't a content management system?**
...
What is that one problem OpenGraph solves? It helps social media apps and sites generate pleasing little clickable info cards from links. Every social media site presents the information a little differently, but they all try to pull the data via OpenGraph, and only when that fails do they fall back to some kind of content analysis, or they just present a generic card.
...
**BART: It's not obvious (other than #3) what metadata is creating the different titles. Same question on the earlier example)
