...
- Bootstrap uses component-scoped variables each of the components described in the *Components* section.

As a general guideline, when you're using Bootstrap to style content-oriented sites you'll be focusing mostly if not completely on the global variables, and when you're styling web apps you're mostly be focusing on the component-specific variables. Since we have worn both of those *hats* throughout this series, we'll explore both of these use-cases in our examples.
...
1. They provide access to fine-grained parts of the Bootstrap style within your own CSS code — allowing you to seamlessly blend your styles into the Bootstrap styles
   1. They allow some aspects of Bootstrap's appearance to be customised.

The vast majority of the variables serve both roles, with one notable exception — the break-point variables are provided purely for consumption, and editing them has not effect on a page's behaviour. These breakpoint variables are not even listed in the documentation, but you can see them in the browser's developer tools:
...
We're going to start with the globally scoped variables because they're the simplest to understand, and by definition, have the most wide-ranging effect, we'll start there. Given we know these are the most relevant to content-oriented sites, we'll use a traditional web page rather than a web app for this first worked example.
...
As an example, let's use Bootstrap CSS variables in conjunction with our own style declarations to re-build the quotation page example in the previous instalment using Bootstrap. We'll tweak it to match the PBS house style.
...
We can now start to apply our personality to the site by re-defining all the relevant global Bootstrap CSS variables in the `:root` scope. Because we're only using a very small subset of Boostrap's features in this simple page, there are just three relevant variables to update:
...
We've now done what we can via the Bootstrap CSS variables, so we need to use plain old CSS for the rest:
