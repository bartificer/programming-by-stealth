...
1. There is zero CSS code **BART: how is there no CSS if you're using CSS @ properties?** in this file, every aspect of the calculator is styled using the standard Bootstrap style classes.
...
**BART: Is extant the word you mean? It's an adjective defined as " (especially of a document) still in existence; surviving: *the original manuscript is no longer extant*."**
...
Sure, we could use `color-mix()` to compute most of them, but we would have to do that! We would need to decide on the formulate to use, and write the CSS code — it would not just happen automatically like it does for similar customisations in SASS. **BART: "formulate"? maybe format? or formula?**
...
Simply put, costuming Bootstrap in SASS is orders of magnitude quicker and easier, and you can do a lot more to boot! **BART: "costuming"? do you mean as in dressing it up?**
