...
date: 2025-02-01
---
We're going to start the meat of our exploration of GitHub Pages by looking back slightly and closing a circle we've left hanging for some time. Most of this series has focused on learning client-side web technologies, and the pinnacle of our journey to date was building client-side web apps with HTML, CSS & Javascript using powerful third-party libraries like jQuery, Bootstrap, and Moustache. Then we changed tack to cover [Bash](./pbs143), [jq](./pbs155), [YAML](./pbs168), [MVC](./pbs170) & [Git Submodules](./pbs172). The last web app we built as a worked example was in [instalment 138](./pbs138), and at that point we had just learned how to use NodeJS and NPM in combination with Webpack to avoid the need to link to outside dependencies, but instead, bundle the exact desired versions of all our dependencies into new versions of our apps ready for publishing. But that's where we stopped — figuring out how to actually publish such an app was left as an exercise for the reader!

In this instalment we'll finally learn how to use GitHub Pages to publish such client-side web apps to the internet for free.

What's more, we're going to leverage GitHub's powerful CI/CD (*Continuous Integration/Continuous Development*) tool GitHub Actions to automate the process.

We've mentioned CI/CD in passing in a few recent instalments, but only in very vague hand-waving terms. In this instalment we're finally going to dive in and implement a simple CI/CD pipeline with GitHub Actions to publish an example client-side web app to the internet via GitHub Pages each time updated code is pushed to the `main` branch.
