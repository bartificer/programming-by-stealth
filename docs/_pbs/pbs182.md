...
For example, were we to have chosen to use the green from the Bartificer Creations logo for success rather than Bootstrap's default green, we would have needed tens, or perhaps even hundreds, of style decorations that explicitly assign the value `#93c020` to various elements. Now imagine we also wanted to use the orange from the Bartificer Creations logo as our colour for warnings, now we would have another few tens or hundreds of statements hard-coding `#f19200`. Just having all those *magic values* would make our style sheets difficult to read, debug, and maintain, but now imagine changing our minds, deciding to use a different green or orange!

Clearly, we need to be able to defined some form of **re-usable named values** in our style sheets! In any other programming language we would call these named values *variables*.
...
To set the `color` property to `DarkBlue` for all top-level headers we would use:
