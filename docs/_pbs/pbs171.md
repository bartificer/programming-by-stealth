...
Time for the implementation of the Model View Controller pattern.
...
==diagram is missing alt text==

![MVC-diagram](../assets/pbs171/mvc-diagrams.svg)
...
==I'm already lost. I don't see the `lib` directory. I have to go poking into directories to find it. I found it in `src` so that must be what you mean when you said "directory of the source code". That wasn't obvious.  I go into src/lib, and I see 10 files. I thought I would see the source code. Evidently it's piles of files. Next you're talking about a gateway - what's a gateway? and then a gateway is a design pattern??? I see a file called xkpasswd.mjs but that's not the class I guess because you capitalized it. so ...?==
...
So back to our diagram, the cylinder contains all of the classes in the `lib` directory which are a black box to the other code with the `XKPasswd` class as the only entrance.
...
This gateway setup also allows us to keep the model and the various views (both web and command line) in one git repository and still build various versions of the application.
...
![Diagram of the directories](../assets/pbs171/directories.svg)
...
![Plotting the views and controllers on the UI](../assets/pbs171/xkpasswd-ui.png)
