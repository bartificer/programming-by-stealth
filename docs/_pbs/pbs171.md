...
**the above sentence is too early - we don't know wht the MVC pattern is**

All these classes are initialized in `index.mjs` in the `XKP` object. This object serves as a wrapper around all the code in the `index.mjs` file to prevent it from being in the global namespace.

**Don't know what this means. how? we don't know anything about index.mjs yet**
...
**why not put the three classes in order of M, V, and C?**
...
**grey text on orange boxes is hard to read**
...
Finally, because the classes in the `lib` directory only deal with the business logic, it is very easy to test them with Jest tests. Writing tests for UI manipulation is the hardest part of testing. By separating the Model from the View and the Controller it becomes much easier to write tests for the Model.

**Suggest we say "unit tests" not "Jest tests". Not helpful really to specify the test tool du jour.**
...
**image(s?) below are unlabled and mysterious! VERY tiny 1 and 2 orange annotations. oh gosh there are green and purple and blue numbers too? it's also too early in the notes, well before you refer to it.**
...
XKPasswd has two main areas in the UI:  the part where the configuration can be changed and the part where the passwords are generated and displayed along with the statistics.  The configuration part can be separated into the preset part and the individual settings part. The classes in the `lib` directory also match this separation.

**I don't see settings or individual settings in the `lib` directory, only presets**
...
To keep the code manageable Settings was also divided into three parts with a View and a Controller pair for each part.

**Now I see settings and presets in `src/web` as views and controllers**

If you look at the screenshot of the UI you will see the Preset part is marked with a green box. The Settings part with a purple box and the Password part with a blue box. These colors match with the colors of the controller/view pairs in the diagram above. **the original diagram?**

**This screenshot is truly difficult to see. I was about to tell you there is no green box because I couldn't see it. I can try to help make one that's readable. **
...
**I put "the Gateway class" in parentheses because I thought it was referring to the `XKPasswd` class but maybe it wasn't? Does the `XKPasswd` class actually handle this?**
...
**where was the PasswordController in the example above?**
...
In June of 2024 Luis Tavares, also known as _[irsheep on GitHub](https://github.com/irsheep)_, created a pull request to add a much-requested feature to XKPasswd: a way to store and retrieve the current settings. His implementation was to create an encoded blob that contains the settings and add that as a query parameter to the url of the XKPasswd website. You can copy that url and bookmark it and Luis' code will decode the blob and fill the settings according to your preferences.

**where can we copy that url and bookmark it?**
...
Luis' code was basically doing the same thing but starting from a different position. He created a Config class, so, why not combine the different ways to save and reuse a specific setting? The code is rearranged to create a ConfigView and a ConfigController class.

**What code is rearranged? "reuse a specific setting"?**

It doesn't matter that the import and export of the JSON files are in the menu and the settings link is under the Settings box. That is for the ConfigView to handle. **lost you here**

The ConfigController **where is this? it's not in `src/web` with the other controllers** handles the reading and writing of the JSON files and the encoding and decoding of the settings link. It also tells the SettingsController **there is a settingscontroller but not with leading caps** to display the custom settings. Finally, it takes care of updating the settingslink **no settingslink either and you've never mentioned it before** So whenever the settings change, the SettingsController calls the ConfigController to update the settings link.

If you look at the image **what image?** you will see two orange boxes around the menu items and the settings link. In the diagram, there is an orange Controller/View pair. **why are you telling us this? what's the point you're trying to make here? what controller/view pair?**

At the time of this episode, this pair is slightly paler than the other pairs, because it's not yet fully working. **is that why it's hard to see the grey text on orange?** That is, the functionality is available on the website, but it hasn't been tested yet by many people.

**where's the ending? no wrap up? no "here's what we learned?" It just kinda stops...**
