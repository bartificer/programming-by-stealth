...
When you get tired of scrolling and of losing hair from scratching you will start to break up long functions in to shorter ones, but then you also need a kind of table of contents of all the functions you've created, because there will be many. 
...
And it becomes almost impossible to unit test a function if that function also manipulates the UI, e.g. one function handles the button press, generates the passwords AND updates the text area with the passwords.
...
So back to our diagram, the cylinder contains all of the classes in the `lib` directory which are a black box to the other code with the `XKPasswd` class as they only entrance.
