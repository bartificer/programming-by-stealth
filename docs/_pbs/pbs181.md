...
**BART - 2nd sentence is long and convoluted - don't understand it well enough to fix. Maybe make more than one long sentence.**
...
Now that since our list of pages to link to is in the `nav_pages` variable, we can loop over them with a Liquid `for` tag of the form:
