...
Using the demo site as it currently stands at the end of this instalments worked examples, add a reusable snippet for creating nicely presented side-notes to any of the site's content. The note needs to receive its contents as an argument named `content`, and it should correctly handle Markdown within that content. The note should be rendered using a [Bootstrap 5 card](https://getbootstrap.com/docs/5.3/components/card/). 
...
To make sure your snippet works well, add one or more notes to one or more pages. 
