...
- The instalment ZIP file — [pbs177.zip](./assets/pbs177.zip) TO UPDATE
...
Before we add the Bootstrap code, let's take a moment to learn about two standard folder locations used by Jekyll that relate to CSS and Sass files.

1. The `assets` folder should be used to store files referenced in your content like Javascript files, CSS style sheets, and images. By convention, `assets/css` is used to hold a site's CSS, and `assets/js` is used to hold a site's JavaScript.
2. The `_sass` folder is used to store **Sass Imports**, and only Sass imports.

One last important piece of information to know is that **assets with the `.scss` file extension that contain blank front matter are compiled to CSS automatically**. This means that a file `assets/main.scss` with blank front matter will get compiled to `assets/main.css` using Sass.

So, to be able to import the Bootstrap Sass files in our own Sass files we need to save them somewhere in the `_sass` folder. Similarly, to be able to include Bootstrap's JavaScript we need to save those files somewhere in the `assets folder.
...
Next, create a file named `docs/assets/css/style.scss` with the following contents:
...
At this point, if we asked Jekyll to re-build our site all of Bootstrap would appear in our output site as `assets/css/style.css`. The problem is this is not compatible with the default Bootstrap theme which uses some of the same CSS class names as Bootstrap. This means that to use Bootstrap we actually need to replace the default theme with our own theme. This is what we want to do anyway, so let's get the ball rolling on that with the most simplistic theme possible that just adds a browser window/tab titles, includes the Bootstrap CSS, and the rendered HTML version of the markdown from our content files.

This is actually surprisingly easy to do, simply create a file named `docs/_layouts/default.html` with the following content:
...
This is mostly a regular HTML file, but it does contain a few Liquid template strings. For now, I'm going to ask you to accept this syntax as magic sauce that makes the theme work, but we will loop back and fully explain this syntax in an up-coming instalment.
...
The complete file should now contain:
...
This instalment was about building a scaffolding more than anything else. Now that we know how to get a basic Jekyll site with a custom theme that uses customisable Bootstrap up and running, we're ready to start learning how to use Jekyll for realistic sites.

If we stretch the building analogy a little, this instalment was site preparation work, in the next instalment we'll pour a solid foundation by building a genuinely useful starter theme. We'll continue to build on this theme as we continue our explorations. The main focus of the next instalment will be the fundamental structure of a Jekyll theme, but to facilitate our examples we'll also spend some time learning about Jekyll's most basic taxonomy, stand-alone pages.
