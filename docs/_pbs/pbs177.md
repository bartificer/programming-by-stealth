...
Finally, create a very basic home page by creating a file named `docs/index.md` with the following content (included in the resources as ``):
...
2. The `_sass` folder is used to store Sass **imports** (not your main Sass files).

There are two important things you need to understand to be able to use these two folders correctly with compiled CSS:

1. **Assets with the `.scss` file extension that contain blank front matter are compiled to CSS automatically**. This means that a file named `assets/css/style.scss` with blank front matter will be automatically compiled to `assets/css/style.css` using Sass when Jekyll builds the site.
2. The entry points to your Sass code must be assets, so sticking to best practice that means you add your primary Sass file(s) to `assets/css`. Within those primary Sass files, each time you call `@import`, Sass will try to find the requested file in the `_sass` folder.
