...
Begins at: [Worked Example — A Multi-Section Web Page](#instalment-resources-only-needed-for-part-b)

<audio controls src="https://media.blubrry.com/nosillacast/traffic.libsyn.com/nosillacast/PBS_2026_05_23.mp3?autoplay=0&loop=0&controls=1">Your browser does not support HTML 5 audio 🙁</audio>

You can also <a href="https://media.blubrry.com/nosillacast/traffic.libsyn.com/nosillacast/PBS_2026_05_23.mp3" >Download the MP3</a>

Read an unedited, auto-generated transcript with chapter marks:  <a href="https://podfeet.com/transcripts/PBS_2026_05_23.html">PBS_2026_05_23</a>
...
In the `<head>` section it simply defines the usual metadata, and then tries to load the generated CSS from `index.css`:
 ...
 * - Indie Flower     : a handwriting font that compliments the PBS site typography
 */
...
Now that we have our generated CSS, open `index.html` in your favourite browser to see our un-styled starting point.
...
Note that Allison got slightly different output on her Mac, so while the important information will be the same, the details may vary. This is what Allison saw:
...
// Typography Variables - capture the PBS house style
...
2. I reused the value of the `$logo-color` and `$secondary-color` variables in later variable definitions. The order is important; you can't use SASS variables before you assign them values!
...
You'll notice that literally nothing has changed! This is because we've only declared variables, we haven't actually used them to declare any styles yet!
...
1. Add all margins on a single side, the bottom
...
@each $heading, $size in $heading-font-size-map {
  #{$heading} {
    ...
    color: $heading-text-color;
    font-size: $size;
    @include block-spacing(true); // pass the optional argument to use header spacing
  }
}
```
...
4. Style the code snippets so they use an appropriate font, and so they stand out appropriately 
...
header {
  ...
  section {
    ...
    border-radius: 0.5em;
    padding: 1em;
    background-color: color-mix(in srgb, $logo-color, white 90%);
...
article#wisdom {
  blockquote {
    ...
    color: rgb(from $logo-color r b g); /* use the CSS function rgb() to swap the blue and green channels of the logo colour */
...
```scss
$handwriting-text-color: rgb(from $logo-color r b g); /* use the CSS function rgb() to swap the blue and green channels of the logo colour */
// add additional global colour-related variables here as needed
```

We can no do a find-and-replace for `color: rgb(from $logo-color r b g); /* use the CSS function rgb() to swap the blue and green channels of the logo colour */` and replace it with simply `color: $handwriting-text-color;`!
...
This has been a whistle-stop tour of SCSS. Hopefully you've picked up enough to start using this powerful technology in your own projects. Additionally, this instalment should have armed you with enough of a grasp of the concepts and jargon to be able to find what ever additional details you need in the documentation or from your favourite code assistant.
