...
* **Selector nesting** — using curly braces to control the targeting of style declarations more clearly. R?esults in easier to write, read, debug, and maintain style sheets.
...
* **SASS Operators, Functions, and Modules** — as we explore variables, conditionals, and loops we'll slowly meet the arithmetic, string, and comparison operates SASS provides, as well as some of the functions provided by some of SASS's built-in modules. These functions fill in some of CSS's gaps and pain-points, and they act as a kind of code lubricant to help you get what you need with less effort. Individually, none of these are all that dramatic, but taken together they're genuinely valuable.
* **Mixins** — SASS's implantation of templates. This is the most advanced feature we'll cover. You may never end up writing your own, but if you want to expand Bootstrap by adding new variants of the existing components, or adding entirely are components, they're vitally important. As you read the SASS sub-sections scattered throughout the Bootstrap documentation, you'll notice two headings that show up repeatedly — *Variables* & *Mixins*!
...
8. **SASS at-rules** — special SASS statements that look like CSS at-rules but only exist in SASS. These SASS at-rules  provide SASS-specific functionally. That ever it is these statements do, they all get converted to regular CSS code at by the SASS compiler in some way. Notable examples we'll be exploring are `@extend` for style inheritance, `@if` for conditionals, and `@each` for looping.
...
**All valid CSS data types are valid SASS data types**. In other words, SASS can handle colours like `#c0c0c0` & `DarkGray`, dimensions like `45px`, `24%` & `3rem`, bare keywords like `solid`, numbers like `42` & `3.1415`, and strings enclosed in single or double quotes (which ever is most convenient in any situation).

What SASS adds to the mix is two important data types CSS does not support — lists and dictionaries. 

The official SASS jargon refers to what we call lists as *Lists* (convenient for us!), and what we call dictionaries as *Maps*. This is actually a nice term for dictionaries because they provide mappings between names and value. Its also a term we've seen before, in our exploration of [jq](https://jqlang.org)!

### Comments — SASS+CSS or SASS-Only

SASS supports two types of comments, CSS commends, and SASS comments. While neither get executed by the SASS compiler, it does treat them differently.
 ...
 * ==========================
 * This is a CSS style sheet that does absoltuely nothing but was generated from a SASS file.
 */
...
This simple little test verifies that you've successfully installed `sass`. It also illustrates the difference between how CSS and SASS comments — the CSS comment appeared in the generated CSS, but the SASS comment didn't.

## A Quick SASS Oview

BART LEFT OFF HIS PROOF READ HERE!!!

To prepare four our worked example, let's run through the SASS feature we'll be covering with simple little examples to illustrate each as we go.
