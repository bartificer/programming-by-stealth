...
1. `contrib` — a folder containing a copy of utility code from other sources, in this case, just the latest free version of [FontAwesome](https://fontawesome.com/) download from their website.
2. `index.html` — the example web page we'll be styling. This page:
   1. Imports the FontAwesome web font from `./contrib/fontawesome/css/all.min.css`
   2. Imports its style sheet from  `./index.css` (which does not exist yet)
...
2. Set the typography for our headings
...
p { 
  @include block-spacing; // don't pass any arguments
}
...
Notice how we are using the mixin — for paragraphs, we are treating the mixin as if it has no arguments since we just want to use the defaults, we just call it by name and omit the parentheses (`@include block-spacing;`), but for the headings we do need to pass an argument, so we do have to include the parentheses (`@include block-spacing(true);`).
...
Notice we are using interpolation to use the keys from the map as the selectors for the six style declarations (`#{$heading}`). Save the SASS file and let the CSS rebuild, you'll see the loop produced the following CSS:
...
Notice that just nine lines of SASS code produced all that CSS code! If you ever wondered how Bootstrap's massive CSS file, with all it's tedious repetition could possibly be created and maintained, now you know — lots of SASS loops!
...
TO DO — update this section so it uses the variable and it becomes part of the worked example (it was originally written for the abandoned playground idea)

 making it both easier to define styles that should only apply to nested sub-sets of a page, and, makes the code much easier to read, so it's clearer what styles are intended to apply where.
...
### Customise the First Section

TO DO

### Customise the Second Section

TO DO

### Use a SASS Mixin to Add FontAwesome Icons to all Code Snippets
