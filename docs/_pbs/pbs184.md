...
Unlike CSS custom properties, SASS variables really do behave like the typical variables in regular programming languages. Their scope is even (mostly) defined by curly braces (more on that later!) **BART: I looked it up; in both British and American English punctuation goes outside the brackets by default UNLESS it belogs to the parenthetical sentence. In this case, "more on that later" is what's being emphasized, unless you're super excited about scope being defined by curly braces ;-)**
...
* Four of the five basic arithmetic operators have operators:
  * `+` & `-` for addition **BART: and subtraction? also how is that + different than concatentation?**
  * `*` for multiplication
  * `%` for modulus (integer division remainder)

So what's missing? **There is no division operator** (or rather, there was one, but it's now deprecated, so you should pretend there isn't!) **BART: same rule**
...
* SASS offers the usual exponential functions like `math.sqrt($number)` to get the square root of a number, and `math.pow($number, $power)` to raise a number to any power. **BART: SASS offers it, or the Math module offers it?**
* `math.abs($number)` get's the *absolute value* of any number, i.e. forces it to it's positive version.
* SASS offers a nice suite of rounding functions: **BART: same Q**
...
But you can do more with variables than that! You can actually use SASS variables just about everywhere ([see the documentation for full details](https://sass-lang.com/documentation/interpolation/))! **BART: exclamation point correctly outside.** The complication is that when using them in any context other than a CSS value, you need to use the interpolation operator (`#{}`).
...
Save the following to a file named `temp2.scss` :
...
```sh
npx sass --stdin --no-source-map < temp2.scss > /dev/null
```
...
Save the following to `temp3.scss`:
...
Running this with the same command we used for lists (`npx sass --stdin --no-source-map < temp3.scss > /dev/null`) produces the following output:
...
To see how conditionals can be used to conditionally include some style definitions, save the following as `temp4.scss`:
...
```sh
npx sass --stdin --no-source-map < temp4.scss
```
...
As an example, save the following as `temp5.scss`:
...
Then compile it with `npx sass --stdin --no-source-map < temp5.scss > /dev/null`. The output will be:
...
As an example, save the following as `temp6.scss`:
...
Again, compile this with `npx sass --stdin --no-source-map < temp6.scss > /dev/null` and you should see the following output:
...
As an example, save the following as `temp7.scss`:
...
Again, compile with `npx sass --stdin --no-source-map < temp7.scss > /dev/null` and you should see the output:
...
**BART: Mixin seems to be changing capitalization in the prose - should it always be leading cap if not in code?**
...
2. The default values for arguments are optional, but **arguments without default values are required**! **BART: huh?**
...
To see mixins in action, save the following as `temp8.scss`:
...
```sh
npx sass --stdin --no-source-map < temp8.scss
```
