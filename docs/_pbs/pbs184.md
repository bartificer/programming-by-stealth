...
When developing it can be quite frustrating to keep manually re-compiling your SASS code each time you make a change. This is why I prefer to use `sass`'s watch mode to detect changes to my SASS files automatically, and re-build the CSS each time the file changes. The syntax for *watch mode* is a little different to the regular syntax:
...
Now that we have our generated CSS, open `index.html` in your favourite browser to see our un-styles starting point.
...
Save the file, and you should see the still running `sass` command detect the file change and re-build the files:
...
Save the file, allow SASS to re-build the CSS file, and then refresh your browser.

You'll notice that literally noting has changed! This is because we've only declared variables, we haven't actually used them to declare any styles yet!
...
2. I also choose to adopt Bootstrap's guidance for handling spacing (margins and paddings) — I zeroed both the margin and padding. It's easier to control overall page layout when the `<body>` tag has no spacing.
...
We have two top-level container tags in our markup — the `<header>` tag containing the page header and the instructions, and the `<main>` tag containing the rest of the content. We need to add some margins top-and-bottom to both separate the containers from each other, and, to separate them from the top and bottom of the page's body. Again, rather than re-inventing the wheel, I've simply adopted Bootstap's approach. I added top and bottom margins of `1.5rem`, zeroed the left and right margins, and then zeroed the top and bottom padding, but added `1.5rem` of left and right padding. This means our boxes touch the sides of our browser window, but are vertically separated from each other by margins, which collapse, and our text is prevented from touching the edges with padding. Reminder — when margins touch, the merge into a single margin with the maximum value of both. This means we get equal vertical separation between the top and bottom of the window, and each top-level block-level elements.
...
For now, we're only concerned with headings and paragraphs, but we know we'll need to apply this same approach to other tags like block quotes and definition lists later in the process. This gives us a a perfect opportunity to define a simple mixin.

We need our mixin to:

1. Always remove the padding
2. Remove all margin but the bottom, and set that to `0.5rem` for headings, and `1rem` for everything else.
...
Rather than re-inventing the wheel I'm replicating Bootstrap's approach yet again!
