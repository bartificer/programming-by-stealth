...
To prepare for our worked example, let's run through the SASS feature we'll be covering with simple little examples to illustrate each.

### Selector Nesting

One of SASS's most useful features is the ability to nest style decorations. In regular CSS, when you want to target a suite of styles to parts of a page contained within other parts of the page you need to duplicate the nesting for each selector.
...
This works just fine, but it's not great for developers for tworeasons:
...
But you can do more with variables than that! You can use actually use SASS variables them just about everywhere ([see the documentation for full details](https://sass-lang.com/documentation/interpolation/))! The complication is that when using them in any context other than a CSS value, you need to use the interpolation operator (`#{}`).

One of the most common places to see interpolation used is in CSS style property names. As an example, let's use variables to add a border to block quotes that will have a default border style with on three sides, and then a different width border on one side, but we want the side to be configurable. That means we need to vary the **CSS property name** itself, not the value it gets assigned. CSS provides the property names `border-top`, `border-right`, `border-bottom` & `border-left`, so we want to use a variable to store one of `top`, `right`, `bottom` & `left`, and have that variable control the side that gets treated differently (which we'll name `$border-highlight-side`):
...
That is to say, **lists cannot be edited** once they are created. This means that list-altering functions like `list.append()` & `list.join` actually create entirely new list, which is why we need to explicitly save them back to a variable!
...
BART LEFT OFF HERE PROOF READING!!!
