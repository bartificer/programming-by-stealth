...
3. You can host the kind of client-side web apps we've been developing in this series without even learning about Static Site Generators.
...
1. The [Liquid templating engine](https://shopify.github.io/liquid/) created by Shopify with [Jinja2](https://jinja.palletsprojects.com/en/stable/templates/) data processing operators and filters
2. The CSS pre-processor [Sass](https://sass-lang.com)

GitHub Pages in no way enforces the use of Bootstrap, but given that we've relied heavily on it throughout this series, and that it's customisation features are implemented in Sass, we'll be using [Bootstrap 5](https://getbootstrap.com/docs/5.0/getting-started/introduction/) with GitHub Pages in this series.

Putting it all together, you see we have a mix of technologies we're already familiar with and new ones we'll need to learn:
...
Our approach in this series will be to start with a Quick Look at using GitHub Pages to host a the kind of HTML+CSS+Javascript web apps we were developing earlier in this series. This use of GitHub Pages ignores Jekyll completely, but allows us to explore how GitHub Actions work.

With that done we'll start on the real meat of the series — learning how to build GitHub Pages sites using Jekyll's content management features. We'll build this up gradually, and we'll do it in such a way that we can test our sites locally before we push our changes to GitHub to publish our sites.
...
Finally, we'll explore Jekyll themes, allowing us to understand how themes developed by others work, how to extend existing themes to customise them, and how to create our own themes from scratch.
