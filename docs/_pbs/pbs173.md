...
To explore Git submodules we're going to pretend to be a small business that looks after two web apps, both of which share the company's branding.

To keep things consistent, the branding will be versioned in its own private Git reposutiry named `pbscorp-brand`. Each web app will be in its own repo, rather unimaginatively named `pbscorp-app1` & `pbscorp-app2`. Both app repos will link the brand repo as a submodule.

To play along, start by downloading the instalment ZIP file, extracting it, and opening a terminal in the extracted folder.

To save time, run the shell script `initPBS173Demo.sh` with the commands:
