...
Your most likely first intruduction to submodules will likely be cloning an existing repository that makes use of them, maybe in work, or maybe some open source code. Without a basic intruduction to submodules this can be a very frustrating experience, because at first glance, it will appear the code in the repository is broken! But don't worry, just a few simple Git commands will get things running smoothly for you.
...
In the real world the URL for the repository we wish to clone that happens to use submodules will most likely be a genuinely remote repository that we connect to over SSH or HTTPS, but in our example we'll use a simple relative file path as the URL, specifically, we'll use `../remote-repos/pbscorp-app1`.

We can clone the repo as normal with the command (remember to make sure you are in the `pc-app1Dev` folder):

```sh
git clone ../remote-repos/pbscorp-app1
```
...
Before we resolve our issue, let's explore this repo a little more so we can learn how to tell we have an incomplete clone with un-initialised submodules. Start by changing into the newly cloned repo with the command:

```sh
cd pbscorp-app1
```

You might imagine runing `git status` to see the state of the repos would show us we have a problem, but it would not, it simply outputs:
...
[submodule "brand"]
	path = brand
	url = ../../remote-repos/pbscorp-brand/
```
