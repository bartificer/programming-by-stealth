...
For example, To loop over a list of all static Javascript files in a site we can use the `assign` tag to create a new variable named `js_files` by filtering the list of all static files in the site (`sits.static_files`) down to just those with a file extension (available via the `extname` key) with the value `js` using the `where` filter. Note that the `where` filter takes two arguments — the name of the key whose value should be checked, and a required value for that key. Only items in a list where the value of the given key matches the given value will be returned by the filter.
...
## Worked Example — Making the List of Pages Dynamic

TO DO — add description of the problem to be solved.
...
Let's finish by highlighting a few key points:

1. White space control — TO DO
2. Use of the `assign` tag to save the output from a filter — TO DO
3. Use of the `where` filter to filter a list based on the value of a key — TO DO
4. Use of the `for` tag to iterate over the filtered list previously saved — TO DO
5. Use of the `relative_url` filter to ensure the links work both during local testing and on GitHub — TO DO
