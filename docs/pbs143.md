...
As mentioned in the introduction, the first line of a shell script tells the OS what shell to use to execute the remaining lines of the script with. For reasons that are still very much debated, this is called a [shebang line](https://en.wikipedia.org/wiki/Shebang_(Unix)). We'll look at the best shebang line for our scripts in a moment. Each line after the shebang line is treated as a separate command, though blank lines are ignored.
