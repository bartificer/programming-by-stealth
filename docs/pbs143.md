...
You'll find more details in [instalment 12 of the Taming the Terminal series](https://ttt.bartificer.net/book.html#ttt12), but the basics of shell variables is that their default scope is the script they are defined in, they're created by assigning them a value with the `=` operator, and they're accessed by pre-fixing their name with the `$` operator.
