...
> Almost everything in this series is powered by open source technologies. Even this silly little demo uses Bootstrap 4.

This is correct but not complete, since Bootstrap 4 uses both jQuery & Popper.js, the paragraph really should mention that, so, there is a fix for this sitting in a branch named `fix_missingOS`. Let's check that out and have a look:
...
> Almost everything in this series is powered by open source technologies. Even this silly little demo uses Bootstrap 4, and by extension jQuery and Popper.js.
...
As expected (and planned), we have a merge conflict in `index.html`. Note that the terminal output very clearly tells you that there is a conflict, and which file or files are conflicted.

The repository has now entered a special state, the merge has started, but not completed, so the working copy is in a kind of limbo between the before and after universe. We now need to work through each conflict and resolve it before we can complete the merge and move beyond this limbo state back into a clean working copy. There's no limit on how long your repository can stay in this state. If you were merging a huge change in a massive project you might have hundreds of conflicts to resolve, so you could be stuck in this state for weeks!
...
The way Git tells us that we are in the conflicted limbo state is by saying we have *unmerged paths*. The same terminology is used as the heading for the list of files that have conflicts, in this case it's a list of one, `index.html`.
...
We now know there is a conflict in `index.html`, how are we supposed to resolve it?

What Git has done is put both versions of the conflicted section (or sections) of the file into the file, but delimited them with special lines of text.
...
Git-aware code editors can detect these conflict markers and add enchanced UI of their own around them. This is how the conflicted section looks in VS Code:
...
The conflict warning have also vanished in GitKraken:
...
We can now tag our new release and delete our obsolete branch:
...
We can see that all is normal on the terminal:
