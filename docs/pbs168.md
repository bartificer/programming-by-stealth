...
## Testing our YAML (with the help of `yq`)

Before we dive into how YAML works, we're going to need a way of illustrating what data a piece of YAML code represents. Since we're already familiar with JSON, that would seem to be the best choice, so we'll be converting YAML code snippets to JSON to show what they do.

**I've performed all the needed conversions and included the resulting JSON directly into the show notes, so the remainder of this section is entirely optional!**

There are many conversion tools available between YAML and JSON, including some free online converters, but because we've just finished learning so much about `jq`, I've decided to use a related tool — `yq` for the YAML → JSON conversions in this series.

This tool still under active development, and not quite feature-complete yet, but it's already very usable and it allow the `jq` syntax to be used to query an impressive selection of commonly used data and config file formats including YAML, JSON, XML & CSV. This command is written from the ground up in the modern Rust language, so it has no dependencies!  You can get all the details on their [GitHub page](https://github.com/mikefarah/yq).

On the Mac you can install it via HomeBrew (like we did `jq`) with the command:

```sh
brew install python-yq
```

Note that the various YAML snippets have not been saved as separate files, instead, I simply piped my Mac's clipboard contents straight into `yq` using the Mac terminal command `pbpaste` :
...
> _**Note:** There are actually two tools with that name, one is a simple Python-based wrapper around `jq` that simply converts YAML to JSON using Python's native libraries, and then passes that JSON to the `jq` command for processing and rendering. This is a rather crude tool, but it does have the advantage of having feature-parity with `jq` because it is in fact `jq` under the hood! If you want to give this a go, you can find out more on their [their GitHub page](https://github.com/kislyuk/yq)._
{: .aside}
...
## MAYBE Using `yq` to Query YAML Files
