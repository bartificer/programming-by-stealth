...
Secondly, so-called _**front matter**_ has become the defecto technique for embedding metadata within Markdown files, and those front matter blocks are in fact YAML.
...
Fixed-width files are not so much a format as an approach. In effect, every fixed-width text file is its own format — the way it works is every line a text file is a collection of fields, and everyone agrees up-front what fields each line will contain, and exactly how many characters wide each field will be. If your data is shorter than the agreed field width you **must** pad you data with spaces.
...
Because fixed-width fields are so difficult to work with they were soon followed by **Tab-Separated Value** (TSV) files, and shortly after that, Coma-Separated Value files (CSV) files.
...
Because of its enthusiastic adoption by major enterprise vendors like Sun Microsystems (Java), Oracle & Microsoft (IIS), XML is still doing a lot of heavy lifting in large enterprises to this day. With my work hat on I still see XML used in lots of config files, for serialising and de-serialising data to and from disk, and for moving data around with XML-based message passing and remote procedure calls ([SOAP](https://en.wikipedia.org/wiki/SOAP), [XMLRPC](https://xmlrpc.com), etc.).
...
_(**Note:** in its very earliest days YAML was a humours acronym — 'Yes Another Markup Language')_
...
Now that we can represent single values, let's look at the two types of collection YAML provides.
