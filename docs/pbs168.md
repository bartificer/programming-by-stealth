...
As part of our re-building of XKPasswd we're extending our scope to include the developer tools that make real-world software creation possible. At the moment, none of the tools we are using in the XKPasswd project use YAML for their configuration files, but that's likely to change before the project is done, because YAML is becoming an ever more popular choice for configuration files.

YAML is also becoming more and more popular for embedding metadata and configuration options within so-called static-site-generators, these are apps that turn a folder of simple text files, often in Markdown format, into a website. A term you often hear in conjunction with Markdown-based static site generators is *fornt matter*, this is YAML-formatted metadata embedded at the top of Markdown files.

In fact, YAML intersects directly with this series through a static site generator because this website you're reading right now is built using the [Jekyll static site generator](https://jekyllrb.com), which is the open source engine powering [GitHub Pages](https://pages.github.com), where this site is hosted. Jekyll makes extensive use of YAML to store configuration information, for front matter, and for data storage.

YAML is designed to be human readable first and foremost, which probably explains its rising popularity. It was also designed to be extremely versatile, so it can do a lot more than just config files and data storage, but those are the users we'll be focusing on in this two-part mini-series.
...
Before we explore YAML, I think it helps to place it within a larger historical context. YAML didn't arrive into a vacuum, its design was informed by the pain-points exposed by its predecessors. We have, after all, been finding ways of representing data in text files for decades — thank goodness we've learned something along the way!

If we cast our minds back to the very earliest days of computing we find primitive data formats like **fixed-width field text files** are all the rage:
...
Each line in one of these files represents one record, and there was some documentation on someone's desk that defined the lengths for each field in the files. For the example above that documentation would need to specify that the first field is 19 characters long and holds a timestamp in ISO 8601 format, then there must be a space, them the second field is be 8 characters long and holds a log severity, then there must be another space, after which the rest of the line is the log message.

These kinds of formats are very human-readable, but, they're also spectacularly brittle — one line of code somewhere that doesn't validate the length of a piece of data and the entire record is corrupted! Not to mention how difficult it is to change your mind about the structure of the data once even a small codebase has been written — extending or adding a fields requires a careful review of every single line of code that processes the files to make sure all the offsets have been adjusted as needed.

In my entire professional life I have encountered just one production system using fixed-width field text files for data import, and was just as much of a disaster as you'd expect. We did need to alter the structure, and it was indeed an absolute nightmare with obscure bugs falling out of the woodwork for years!
...
A TSV file (also called a *tab file*) looks superficially similar to a fixed-width field text file, but instead of counting characters, the fields are separated by tab characters:
...
This gives us what is now by far the most widely used generic text format for sharing tabular data:
...
Because of it's enthusiastic adoption by major enterprise vendors like Sun Microsystems for Java, Oracle for their various enterprise platforms, and Microsoft for IIS, XML is still doing a lot of heavy lifting in major corporations to this day.
...
YAML builds on JSON but takes things just that little bit further by being even more human-readable, and even more efficient:
...
As I said in the introduction, YAML was designed to solve many problems, not just those we are interested in in this mini-series. Quoting from the [specification](https://yaml.org/spec/1.2.2/) (emphasis added):
...
We'll finish our short exploration of YAML in the next instalment when we look at a few advanced topics, and dive a little deeper into the `jq`-like `yq` command, and use it to process some YAML files.
