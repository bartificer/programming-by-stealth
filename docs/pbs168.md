...
YAML is language for storing data in a simple, human-readable form that can also be reliably processed by computers. It reminds me a lot of Markdown which is design to be human readable and obvious as-is, but can also be read by computers and converted into other formats like HTML for presentation. YAML wants to be the Markdown for data!
...
Allison has also been working on building a documentation site for the XKPasswd project, and she's authored the content in Markdown. Now, she's experimenting with different static site generators for converting it to a nice web site, and many of the possible choices out there make use of YAML.
