...
Also notice that the file list shows the commit message of the last change to affect the each file in the file listing. The license has not been changed, so it still shows *'initial commit'*, but the REAME has been changed, and it shows the commit message I entered above.
...
Again, as mentioned above, the command for managing repositories is `repo`, and the sub-command to create a new repo is `create`. You can get the full documentation here, but we can interactively create a repo with `gh repo create`, passing the name of the repo as the first argument. I like to use the `--description` flag to pre-populate the description, so in my case I used the following command to create a repo named `pbs117-helloWorld-cli`:
