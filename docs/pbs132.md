...
For example, the the doc comment for the `Replicator` class has been updated to reference the Star Trek tutorial with a block tag:
...
2. [Docdash](https://github.com/clenemt/docdash)
3. [Clean JSDoc Theme](https://github.com/ankitskvmdam/clean-jsdoc-theme)

Two of the three have sample sites linked from their GitHub pages, but I don't find those very useful, instead, let's test each on our example code. All three have been specified as dev dependencies already, so they were installed when you ran the `npm ci` command to initialise the `pbs132a` folder as a NodeJS project. I also added commands to generate each to `package.json`, so you can generate the three example sites with:
...
This will create three folders, `docs-dev-minami`, `docs-dev-docdash` & `docs-dev-clean`. Open `index.html` in each in your favourite browser to see what they look like.
...
Moving on to Docdash — the sidebar is much better, the nesting is very clear, and the separation between entries within classes etc. is extremely good with those big solid banners.
...
The sidebar again shows the nesting well, and while the collapsible sections seem like overkill on a small demo project, they make a lot of sense for larger code bases.
...
I think it's clear that one of these themes is obviously lagging the others — despite my love of the cute rounded coloured boxes, Minami just isn't up to the task.

That leaves two themes to choose between, and I'm not quite ready to commit to either. Some further experimentation is needed, but one of these two will be the theme we use in this series.
