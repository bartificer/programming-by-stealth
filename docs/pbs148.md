...
We now have a place on GitHub to share our homework. Read about it and join at [Podfeet.com: Introducing the Programming By Stealth Student Organization in GitHub](https://www.podfeet.com/blog/2023/03/pbs-students-github)
...
I also use a `for in` loop to print out the user's order, proceeded with an example of using the `${#array[@]}` syntax for accessing the length of an array for completeness. 

Finally, for reason no one seems to be able to explain, the echo command ignores new line characters by default, so you need to use the `-e` flag to get them to render as newline characters!
