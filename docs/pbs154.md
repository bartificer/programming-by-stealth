...
| [Shell Parameter Expansion](https://www.gnu.org/software/bash/manual/html_node/Shell-Parameter-Expansion.html) | Shell variable value lookups, e.g. `I am $USER` or `I am ${USER}`  →  `I am bart`. Also works with arrays, given `a=(a b c)`; `${a[#]` → `3`, `${a[@]}` → `a b c` & `${a[0]}` → `a` | String                   |
...
| [Word Splitting](https://www.gnu.org/software/bash/manual/html_node/Word-Splitting.html) | The results of all previous expansions get split into separate arguments, or *words* based on the value of `$IFS`, unless they have been quoted | Parameter list           |
