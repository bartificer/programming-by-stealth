...
The only change is the addition of a new rule for the file extensions used by the Bootstrap Icons, and the addition of extra comments throughout the entire `rules` section to clarify what each rule is doing.
