...
Bash thinks in terms of strings, so rather annoyingly, none of the symoblic operators work as you would expect with numbers, for example `0 = 0.0` is considered false, as is `4 < 10`! When you want Bash to think numerically, you need to use a different set of operators, all of which expect two values to compare:
...
Now, let's look at a typical either/or scenario by adding an `else` command into the mix. The script `bps145b-ifElse.sh` will print a different message depending on whether or not you have a Bash profile file:
...
The `case` command itself a value as the first argument, and then the word `in` as the second and final argument. Executing that command puts your shell into a mode where it will interpret the next line as one or more patterns (in the format used by `grep`) TO DO LINK TO DOCS separated by pipes, and terminated with a `)`. Next come arbitarily many commands which will only execute if the value passed to the `case` command matches one of the patterns. This continues until you either use the `;;` command to signal *'next pattern'* or the `esac` command to end the special mode started by the `case` command.
