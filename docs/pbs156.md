...
You might think this means the second number is a length not and index, but no, and we can prove it by selecting the second to fourth elements instead with:
...
```bash
jq '.prizes[1,-1]' NobelPrizes.json
```
