...
Havibg learned some ways to alter strings in the previous instalment, let's pick things up by exploring some ways to transform arrays.
...
```sh
# -n means expect no input 
# -c means compact output
...
As we learned in the previous instalment, the `+` operator has been overloaded in jq to perform a useful actions for different data types. When both inputs are arrays — it merges them into a new bigger array:
...
While the `-` operator is not overloaded for strings, it is overloaded for arrays, and can be used create a new array with elements removed. Like with `+` both inputs must be arrays. The array to the left of the `-` is treated as the original array, and the elements in the array to the right of the `-` are removed from the original array, if present, to create the output array.
...
Firstly, the simple `unique` function returns the input array sorted with any duplicated elements removed. The input must be an array. 

**Note that the function sorts then deduplicates**, it doesn't just remove duplicates! For exmaple:!
...
This shows that hotdogs and waffles have 7 letters, and pancakes 8. We would expect to always get pancakes in our answer, but only one of waffles or hotdogs when we make the menu unique by name length. Let's try:
...
having now explored manipulating numbers, strings & arrays, let's now move on the the final major data type — dictionaries.

### Adding and Removing Keys

As mentioned earlier, the `+` operators is overloaded, and it can handle two dictionaries as inputs. When you *add* two dictionaries, a new dictionary is created containing the keys and values from both. If both input dictionaries define a value for the same key, the value from the dictionary on the right of the `+` operator is used.

If the dictionaries contain dictionaries, a recursive merge can be done by using the `*` operator instead of the `+` operator.

You might imagine a key could comehow be removed with an overloaded subtraction operator, but alas not, we need to use the built-in `del` function for that. This function requires a dictionary as the input, and a key path as an argument. For example, we could remove the stock key from each item in our menu (`menu.js` from the instalment ZIP) with:
