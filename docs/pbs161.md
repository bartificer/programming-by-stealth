...
In the previous instalment we learned to use jq as a scripting language, so we can now create complex jq filters without them becoming un-readable and un-manageable, which sets us up nicely for learning about the many ways in which jq can manipulate data. This will take a few instalments to get through, but we'll start in this instalment with jq's support for maths. Next we'll look at using jq's assignment operators to write values into dictionaries. And we'll finish this instalment by discovering some of jq's many string manipulation functions.
...
The solution is almost entirely the same as the original example, but with the addition of two additional conditions inside the `select`:
...
These additional conditions convert both the year property of the Nobel prize dictionary and the minimum and maximum year variables to numbers, and then apply greater than or equal to and less than or equal to operators as appropriate.

The challenge offered bonus credit for making the minimum and maximum year arguments optional, you'll find my sample solution for extra credit it in the instalment ZIP as `pbs160-challengeSolition-2.jq`:
...
The first point of note is that because the argument may or may not be passed, we cannot use the short-cut name `$minYear` and `$maxYear`, instead, we have to use the `$ARGS.named` dictionary to access these arguments. If the arguments were passed, then `$ARGS.named.minYear` and/or `$ARGS.named.maxYear` will exist, if not,  they will not.

The second point to note is that I detect whether or not the arguments were passed by using the `has()` function to check if `$ARGS.named` as a key named `minYear` and/or `maxYear`.

The approach I chose to take was to ensure that when either optional argument was not present, the appropriate condition would always return `true`. To achieve this end I first inverted the output from the `has()` function to effectively turn it into a *has not* function using the `not` function. Since the alternate operator (`//`) only executes the filter to the right when the filter to the left returns `empty`, `null`, or `false`, this has the effect of only executing the comparison when the variable exists. When ever the variable does not exist `has()` returns `false`, which the `not` converts to `true` which the alternate operator returns without running the filter to its right.
