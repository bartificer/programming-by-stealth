...
The most coarse-grained difference you can have from computer to computer is that some dot files may only be needed on some of your computers. By default when you run `chezmoi apply` all the targets in your source get outputted to your destination.
...
Next, let's look at how I use the `.needs_git` boolean in my `.chezmoiignore` file to only include over my Git configs when needed:
...
Finally, let's look at how my SSH config file makes use of the `is_mac` boolean to only configure KeyChain integration on Macs:
