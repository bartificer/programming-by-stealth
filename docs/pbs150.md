...
## Accepting Input From the Pipe Inteligently
...
Update your challenge solution from last time to accept the menu via the pipeline, or as a path passed as the first argument, only default to `./menu.txt` when nothing was piped to the script, and there is no first argument.