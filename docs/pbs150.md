...
Notice that output has been bifurcated, some when to the terminal, some to the fine, and none to both.
...
Because this is a very common convention, `getopts` does not get confused by the lone `-` this convention uses to indicate the script should *read from `STDIN`*.
