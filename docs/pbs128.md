...
Because the `import` statement has these two distinct modes, it's really important to understand the difference when reading and writing code. IT all comes down to the curly bracesIf there are no curly braces — **curly braces means named exports, no curly braces means default exports** (or a global import)!
...
In the instalment's ZIP file you'll find three folders, `pbs128a`, `pbs128a`, and `pbs128c`. Each is a NodeJS project.
...
If you try run one of the example before installing the dependencies you'll get an error saying a needed module couldn't be found:
...
When we get further into the HSXKPasswd project I will expect anyone cloning the repo and submitting pull requests to use `npm ci` rather than `npm install`, and I'll be rejecting commits that change `packjage.json` or `package-lock.json` unless the intended purpose of the commit was to install or upgrade a package.
...
Luxon uses a very different philosophy to Moment, so the date manipulation code is completely different. It is however, much more readable IMO:
