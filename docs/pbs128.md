...
It's important to point out an annoying caveat here — each modules is imported based on a *module specifier*, and the official specification does not define the rules for modules specifiers, those are left up to the individual JavaScript engine. There are good reasons for this, but it is annoying, and there are moves a foot to standardise the format as much as possible so as to make code as portable as possible. For now, only a sub-set of the possible formats will work both in NodeJS and in browsers. It is possible to write specifiers that made sense to NodeJS, but can't be interpreted by browsers (the inverse is probably possible too, but I'm not certain of that).
...
## A Note on Module Specifiers

TO DO

## Example 1 — The Simplest Case (A Single Default Export & Import)
