...
The instalment resources contain a script named `pbs120-genIgnoreFiles-1.sh`, this script uses the `uuidgen` command (learn more at its [online man page](https://man7.org/linux/man-pages/man1/uuidgen.1.html)) to generate some random glop (a universally unique identifier) which it redirects into files, and the `mkdir` command to create a directory. In total the script creates the following:
...
TO DO — list all files tracked that should be ignored

TO DO — stop tracking a file
