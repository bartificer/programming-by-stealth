...
Something interesting happens when you include what I'm calling a pass-through function in a chain — the function executes, but next function in the chain is called on the same object — **the instance to the left of the pass-through function is also the one to the left of the next function in the chain!**
