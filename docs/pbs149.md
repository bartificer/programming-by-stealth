...
In the real world this is a surprisingly rare edgecase, but what happens when you use `getopts` in a script that expects both accepts options and regular arguments, and the user wants to pass a regular argument with a value that starts with a `-` symbol? How does `getopts` know the user did not mean that thing that starts with a `-` to be an option?

Well, `getopts` can't possibly know what the user means in that situation, so it assumes the intended regular arument is another option, as we can see with our previous example script:
...
Let's demonstate that with our previous example script:
...
This would successfuly create the folder, add the file (as an empty file) and we'd see that file listed as the output from the final `ls` command.
...
To acutally create the folder, add the file, and list the contents of that folder, we need to use the `--` option with all three commands:
