...
By using the `$()` operator you can loop over the output of a command, one word at a time. Commands that produce space-delimited output are perfect for this, but man commands are smart enough to escape their output when they detect they're being used in a shell script.
