...
YAML provides many ways to render multi-line strings, and they ensure you can sanely indent your markup without having un-intended consequences for the contents of your strings.
...
### Flow Style Multiline Strings (Avoid!)

I'm going to start with the simplest of the syntaxes to read, and while you will definitely encounter it in lots of YAML written by others — I **stronly advise against using it** in your own YAML files!
...
You **start your multi-line strings with one of the two string block operators**, wich is **optionally followed by a chomp indicator**, and then you **start the content of your string on the next line.**
...
We can illustrste this with two YAML documents, one that represents a dictionary of arrays, and one that represetnts and array of dictionaries:
...
> TOML is another config file language that feels like it might be on the cusp of a breakthrough. It stands for *Tom's Obvious Minimal Language*, and it can best be desribed as a modern take on the old INI synax used by Windows before the acursed Registry was added to replace config files:
>
> ```toml
...
# apply a jq fitler to some YAML piped to STDIN
...
There's also an [official documentation site](https://mikefarah.gitbook.io/yq) which contains deeper explanations and more examples, but I've found that it's sometimes a little behind in terms of the full list of supported features and options — that is to say, the actual command can do a little bit more than the current official documentation site says it can!

## Final Thoughts

At the very least I hope you'll now recognise YAML when you come across it in a config file or some front matter, and that you'll now be able edit these files with new found confidence. At best, I hope you'll give serious considering to using YAML as your configuration file format on any new software projects you embark on!
