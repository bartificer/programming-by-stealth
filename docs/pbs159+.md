...
Now, let's expand the dictionary we are building to include Andrea's name as a single key.

To do this we need to add a new key named `name` to the dictionary we're building in the final part of the pipeline, but, the value for this key is going to have to be a complex filter in and of itself.

First, we need to explode the laureates and filter down to just Andrea's entry using our existing knowledge. The following filter will result in Andrea's dictionary in the `laureates` array:
