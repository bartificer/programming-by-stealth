...
Many doc tags expect *type expressions*, these are generally surrounded in curly braces.
...
Type expressions range from the wonderfully simple to the devilishly complex. The good news is that you can build up your level of detail over time as you get more comfortable, and, you can actually define and name your own types definitions, allowing you to avoid duplicating complex type epressions throughout your documentation.
...
It takes a long time to become proficient in complex type expressions, so the most important pages to bookmark in the JSDoc documentation is [the description of the `@type` tag](https://jsdoc.app/tags-type.html) which contains a table showing the different syntax options available for use.
...
I was hoping not to need a config file at all for this instalment, but because we need to use the `.mjs` file extension to keep NodeJS happy, we need to tell JSDoc that is should process files with the `.mjs` or `.jsdoc` file extension. Without a customised `source.includePattern` definition in a config file JSDoc only processes `.js` and `.jsdoc` files, **even if you explicitly specify other files in the arguments**.
...
When you run this command a folder named `docs` will be created, containing the generated documentation site. Open `docs/index.html` in your favourite editor to view the docs.
...
TO DO — a quick walk-through of the site
