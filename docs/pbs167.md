...
Most of the time jq does not need traditional `if` statements, the alternative (`//`) or conditional (`?`) operators are usually sufficient, and they're much simpler than a traditional conditional statement, but, as our challenge solution illustrates perfectly, sometimes things get really messy when you limit yourself to those operators!
...
There is also a shorter form with an implied `else`, but it may not behave quite as you expect. On deeper reflection its behaviour is utterly logical, but I don't find it intuitive — the default `FALSE_VALUE` is `.`. In other words, **when you omit the `else` and the condition is `false`, the output is the input**!
