...
When we looked at the plain and update assignment operators we described there being a path to the left of the assignment operator, and a value to the right. That's true, but it\'s only part of the truth, you can have filters that produce *multiple paths* to the left of an assignment, and all the specified values will all be updated!

For example, we can update the keys in a HIBP domain export from email usernames to full usernames with a single assignment by converting the list of breaches to entries, then setting a new value for every key  with the assignment `.[].key |= . + "@demo.bartificer.net"`, and then converting it back to a lookup table:
...
We can even take things a step further and use a `select` within the left-hand-side to pre-fix the email usernames with the string *NAUGHTY* for all users caught up in more than one breach by using the assignment filter `(.[] | select((.value | length) >= 2) | .key) |= "NAUGHTY " + .` between converting the Breaches to entries, and re-constructing them back into a lookup table:
...
While the possibilities are technically infinite, not every filter will work on the left-hand-side of an assignment — jq needs to be able to figure out a path to each produced value to be able to update it. Your left-hand-side filters must only filter the input data structure down, they can't alter it in any way at all. If you get it wrong and use an inappropriate filter you'll get an error that starts with *'Invalid path expression near attempt to iterate through ...'*.

## Destructuring (Advanced Variable Binding)

Remember, jq refers to setting a variable's value as *binding* rather than *assigning*, because it uses *assignment* to mean setting a value within the data flowing through a filter.

We know we can bind values to single variables with the `as` operator, but we can actually bind values to multiple variables at once if we make sure the structure of the left and right sides of the `as` operator match. This is known as *destructuring*.

LEFT OFF HERE!!!

TO DO — one or more examples
