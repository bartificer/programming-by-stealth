...
TO DO — move dictionary alternative encoding up here
...
This works because when we explode the array of log entries we now get dictionaries with keys named `timestamp`, `severity` & `message` which matches our pattern on the right of the `as`.
