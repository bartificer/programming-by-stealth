...
1. A *Primary* repository on the NAS that you consider the master copy, and that you back up regularly.
...
Of the truly remote protocols, the the original one was built specially for Git and is really quite useless. It requires a dedicated daemon, and offers no access control what so ever. You may sometimes still see it used to publish read-only repositories, and you'll recognise it by its URL scheme of `git://`.
