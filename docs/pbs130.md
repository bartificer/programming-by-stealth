...
1. JSDoc uses  variant of the [JavaDoc](https://en.wikipedia.org/wiki/Javadoc) syntax that's becomes a defacto pseudo-standard generally referred to as *doc comments*, and now supported across multiple languages, including PHP (with [PHPDocumentor AKA PHPDoc](https://www.phpdoc.org)). 
2. *Doc comments* are specially formatted code comments, and they're placed directly before the thing they're documenting in the source code.  Your documentation is right next to your code, so you can updated them in unison with ease.
...
5. JSDoc supports the inclusion of stand-alone pages into the documentation, making it possible to add things like a quick-start guide, contributor agreement, etc..
6. JSDoc supports Markdown.
7. The code intelligence in many Many IDEs understands doc comments, allowing them to show the relevant documentation snippets in tooltips, and to generate better code completion suggestions.
9. Many linters, including ESLint, support doc comments, helping us to keep our documentation complete and accurate.
10. JSDoc supports themes, so in theory, the imperfect default theme could be replaced with a better theme in future, without the need to change any of the documentation by so much as a single character.
