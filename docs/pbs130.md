...
Comments are helpful, and you could make an argument that good comments are great, but any comments are better than nonen.
...
In the instalment's ZIP file you'll find a folder named `pbs130a`. In here you'll find a folder named `src` that contains a slightly updated version of the sleeps to Christmas calculator module from [instalment 128](./pbs128) that's had a few extra functions added, in the root folder, a file named `index.mjs` that uses the module to print the sleeps until Christmas.
...
Next, make `index.mjs` this file is executable with:
