...
> Note the use of the `;` command separator to put two terminal commands on a single line.
{: .aside}
...
So, in this one pipeline we use `xargs` to pass the contents of `STDOUT` from one command as an argument to another, and to trim the leading and trialing spaces around `STDOUT`. 
